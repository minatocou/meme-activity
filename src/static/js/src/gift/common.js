...
var APIHOST = $('.header-container').data('gift') || 'http://54.222.139.99';
...
$.fn.datepicker && $('.input-daterange').datepicker({
    format: "yyyy-mm-dd",
    language: "zh-CN",
    autoclose: true
});
...
function ajaxReq(option){
	...
	option.data.operator = $('.record-user-name').text();
	return $.ajax(option)
			.then(function(data){
				if(data.code == 1){
					try {
...
var tipsShow = (function (argument) {
	...
	return function (text,modalDialog,dur){
		...
		tips.text(text).show().appendTo(modalDialog).delay(dur || 2700).fadeOut( '300' ,function () {
			tips.remove();
		});
	}
}());
...
//显示组装项表格数据
function initBoxAssemble(tmpl,data){
	var _this = this;
	ajaxReq({
		url: _this.getAttribute('data-api'),
		data: data || {sku: $(_this).closest('.modal').data('sku')}
	})
	.then(function(data){
		console.log(data)
		var dataArr = formatList(data.data);
			
		// });
			// var dataArr ={
			// 		2312312:[{prodSku : 111,productBarCode : 222,name : 333},{prodSku : 111,productBarCode : 222,name : 333}],
			// 		24343:[{prodSku : 111,productBarCode : 222,name : 333},{prodSku : 111,productBarCode : 222,name : 333}],
			// 		1111:[{prodSku : 111,productBarCode : 222,name : 333},{prodSku : 111,productBarCode : 222,name : 333}],
			// 		65423:[{prodSku : 111,productBarCode : 222,name : 333},{prodSku : 111,productBarCode : 222,name : 333}],
			// 	};
		var tmplData = {};
		tmplData[tmpl] = dataArr;
		if(tmpl === 'showBoxCode'){
			$('[data-bind="code-category-count"]').text(Object.keys(dataArr).length);
		}
		setTimeout(function(){
			$(_this).closest('.modal').find('.tab-pane.active .data-container').html(parseTmpl({id:'item-detail-tmpl',data:tmplData}));
		});
	});
}


function formatList(data,tmplId){
	...
	if(tmplId == 'showBill'){
		...
		Object.keys(list).forEach(function(key,i) {
			...
			delete list[key].barCode;
			delete list[key].qty;
			$.each(list[key],function(idx,obj){
				dataArr[ k ].detail.push(obj);
			});
		});
		var arr = [];
		Object.keys(dataArr).forEach(function (key) {
			arr.push(dataArr[key]);
		});
		return arr;
	}
	Object.keys(list).forEach(function(key,i) {
		...
		//var arr = [];
		var array = dataArr[ k ] = Array.isArray(dataArr[ k ])?dataArr[ k ] : [];
		$.each(list[key],function(idx,obj){
			array.push(obj);
		});
	});
